...
static std::vector<Test::DtmfSequenceItem> parseDtmfSequence(const std::string &input) {
	...
	for (char c : input) {
		if (c == 'w') {
			if (!buf.empty()) { seq.push_back({delay_ms, buf}); buf.clear(); }
			delay_ms += 500;
		} else if (c == 'W') {
			if (!buf.empty()) { seq.push_back({delay_ms, buf}); buf.clear(); }
			delay_ms += 1000;
	...
	if (!buf.empty()) { seq.push_back({delay_ms, buf}); }
	return seq;
}
...
void TestCall::onCallMediaState(OnCallMediaStateParam &prm) {
	...
	if (test && ci.state == PJSIP_INV_STATE_CONFIRMED) {
		...
		for (auto &m : ci.media) {
			...
			if (m.status == PJSUA_CALL_MEDIA_LOCAL_HOLD) {
				LOG(logINFO) << __FUNCTION__ << " Marking call on hold for id: " << ci.id;

				hold_state = true;
				break;
			}
		}
		// We found an active media after hold
		if (any_active && hold_state) {
			// Reconnect player
			...
			LOG(logINFO) << __FUNCTION__ << " Marking call off hold for id: " << ci.id;
		}
	}
}
